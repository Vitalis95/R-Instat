...
Public Class sdgOptions
    ...
    Private Sub sdgOptions_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        autoTranslate(Me)
    End Sub

    Public Sub InitialiseControls()
        Dim dctDist As New Dictionary(Of String, String)
        ucrButtonsDescribeDisplay.iHelpTopicID = 708 


        ucrPnlBins.AddRadioButton(rdoAutoCalculate)
        ucrPnlBins.AddRadioButton(rdoSpecifyBins)
        ucrPnlBins.AddParameterValuesCondition(rdoAutoCalculate, "checked", "auto")
        ucrPnlBins.AddParameterValuesCondition(rdoSpecifyBins, "checked", "specify")

        ucrPnlBins.AddToLinkedControls({ucrChkCalcByMonth, ucrNudBins}, {rdoAutoCalculate}, bNewLinkedAddRemoveParameter:=True, bNewLinkedHideIfParameterMissing:=True)
        ucrPnlBins.AddToLinkedControls({ucrInputBins}, {rdoSpecifyBins}, bNewLinkedAddRemoveParameter:=True, bNewLinkedHideIfParameterMissing:=True)
    ...
    Public Sub SetRCode(clsNewDoFillingFunction As RFunction, clsNewDummyFunction As RFunction, Optional bReset As Boolean = False)
